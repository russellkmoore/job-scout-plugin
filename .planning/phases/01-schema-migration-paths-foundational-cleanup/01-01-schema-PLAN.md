...
autonomous: true
requirements: [SCH-01, SCH-02, SCH-03, SCH-04, CON-02]

must_haves:
  truths:
    - "scripts/schema.py defines MASTER_TARGETS_VERSION = 4"
    - "scripts/schema.py exports STATUS_VALUES frozenset and normalize_application_status() helper"
  ...
  artifacts:
    ...
    - path: scripts/validate_data.py
      provides: validate_runs_log + ensure_today_subdirs validators
      exports: ["validate_runs_log", "ensure_today_subdirs"]
    - path: scripts/tracker_utils.py
      provides: status validation on append + 16-column row construction
  key_links:
...
Purpose: This plan establishes the v=4 schema substrate every later phase consumes. SCH-01..04 + CON-02 land in a single Wave-1 plan because all three files are tightly coupled (`tracker_utils` imports `schema`; `validate_data` imports `schema`).

Output: `scripts/schema.py`, `scripts/validate_data.py`, `scripts/tracker_utils.py` all at v=4 shape.
</objective>
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test 4: `from schema import STATUS_VALUES; "Active" in STATUS_VALUES and "" in STATUS_VALUES and "Dead" in STATUS_VALUES`.
    - Test 5: `normalize_application_status("dad") == ("Active", True)`; `normalize_application_status("DEAD") == ("Dead", True)`; `normalize_application_status("Dead") == ("Dead", False)`; `normalize_application_status(None) == ("", False)`; `normalize_application_status("") == ("", False)`.
  </behavior>
  <action>
  ...
  <acceptance_criteria>
    ...
    - The verify command above prints `OK` (asserts version, list lengths, column ordering, STATUS_VALUES membership, and three normalize_application_status cases)
  </acceptance_criteria>
  <done>
    `scripts/schema.py` is at v=4 shape: 13 master_targets columns, 16 tracker columns/keys/widths, STATUS_VALUES frozenset with 8 members (incl. ""), and `normalize_application_status` helper exporting the (canonical, was_coerced) tuple per Pattern 5 of 01-RESEARCH.md. The verify one-liner exits 0.
  </done>
</task>
...
<task type="auto" tdd="true">
  ...
  <read_first>
    ...
    - scripts/schema.py (after Task 1 ran — to confirm STATUS_VALUES + normalize_application_status are exported)
  ...
  <action>
    ...
    **Edit 3 — extend the `row_list` literal in `append_rows`.** Find the existing 14-entry row_list (lines 201-216) and append two new `.get()` calls after `row_dict.get("notes", "")`:
    ...
    Note the row construction is now 16 entries — matching the 16-entry HEADERS / COL_WIDTHS imported from `schema.py` after Task 1.
...
with tempfile.TemporaryDirectory() as td:
    ...
    # Test 1+3: append a row with bogus status + source/ats_provider
    ...
    with open(rows_path, 'w') as f:
        json.dump([
            ...
            {'job_title': 'Director', 'company': 'Gamma', 'job_url': 'https://example.com/j/7777777777', 'status': '', 'tier': 'C', 'source': '', 'ats_provider': ''},
        ], f)
...
" 2>&1 | tail -20</automated>
  ...
  <acceptance_criteria>
    ...
    - The verify Python block above prints `OK`: 16 headers, status `'dad'` coerces to `'Active'`, `'Dead'` stays `'Dead'`, source/ats_provider populate cols 15/16
  </acceptance_criteria>
  <done>
    `scripts/tracker_utils.py` validates `application_status` via `normalize_application_status` (warn-and-coerce, prints to stderr) before constructing the row, and emits 16-entry rows including `source` + `ats_provider`. `_write_tracker` and `load_tracker` are unchanged — they auto-handle the 14→16 widening per Pattern 2. The verify block exits 0.
  </done>
</task>
...
|-----------|----------|-----------|-------------|-----------------|
| T-01-01 | Tampering (data integrity) | tracker_utils.append_rows on `application_status` | mitigate | STATUS_VALUES frozenset + normalize_application_status helper coerces unknowns to "Active" with stderr WARNING; case-insensitive match prevents `dead`/`DEAD`/`Dead` from being treated as separate statuses (CON-02). |
...
python3 -c "import sys; sys.path.insert(0, 'scripts'); from schema import MASTER_TARGETS_VERSION, MASTER_TARGETS_COLUMNS, TRACKER_COLUMNS, STATUS_VALUES; assert MASTER_TARGETS_VERSION == 4; assert len(MASTER_TARGETS_COLUMNS) == 13; assert len(TRACKER_COLUMNS) == 16; assert {'Active','Dead',''} <= STATUS_VALUES; print('schema OK')"
...
All three echo lines (`schema OK`, `runs.jsonl OK`, `ats_raw OK`, `tracker_utils OK`) must print.
</verification>
...
- `STATUS_VALUES` frozenset and `normalize_application_status()` helper are exported from `schema.py`
- `validate_data.py` calls `validate_runs_log` from `main()`'s validator list and exposes `ensure-today` CLI subcommand
- `tracker_utils.append_rows` validates `application_status` and emits 16-entry rows
...
<output>
After completion, create `.planning/phases/01-schema-migration-paths-foundational-cleanup/01-01-SUMMARY.md` summarizing the schema bump, the validator wiring, the tracker-row extension, and the deferred concerns (CON-20 stays in Phase 5).
</output>
